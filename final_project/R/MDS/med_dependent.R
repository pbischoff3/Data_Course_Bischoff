...
# Create For Loop for all variables for career  ####
x=1
med_depplots <- list()
for(i in c("si","cm","im","sd","se","gm","sci_comp","per_comm_orient","sci_comm_orient")){
  med_depplots[[x]] <- mysubsetMDS(i) %>% 
...
# Combine all factor plots together ####
med_depplots[[1]]
...
med_depplots[[8]] +
  coord_cartesian(xlim = c(-9.25,-8.25))
...
# well, that didn't work. Let's try a different mds
